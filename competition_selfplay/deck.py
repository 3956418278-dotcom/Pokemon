...
def load_target_deck(config: TargetDeckConfig, root: str | Path) -> TargetDeck:
    ...
    # This branch intentionally has no dependency on the earlier global card
    # feature/vocabulary pipeline.  The fixed deck defines its own compact,
    # deterministic Card-ID vocabulary; 0 and 1 remain reserved for padding
    # and unknown cards at simulator boundaries.
    vocab = {
        card_id: index + 2
        for index, card_id in enumerate(sorted(set(card_ids)))
    }
